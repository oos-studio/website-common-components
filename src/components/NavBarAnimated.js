...
class NavBarAnimated extends Component {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (
      this.state.activeNavImage === this.state.scrollNavImage &&
      !showScrolledNav
    ) {
      runAnimations()
    }
  ...
  runAnimations() {
    ...
    if (showScrolledNav) {
      ...
      TweenLite.to('#navbar', duration, {
        height: styles.scrolled.navbar.height,
        backgroundColor: styles.scrolled.navbar.backgroundColor,
        ease: Power2.easeOut,
      }).then(() => {
        ...
        TweenLite.to('#collapse', duration, {
          ...styles.scrolled.collapse,
        }).then(() => {
          this.setState({
  ...
  renderNavigationItems(item, index, renderImages) {
    ...
    const _styles = showScrolledNav
      ? deepmerge(styles, styles.scrolled)
      : darkMode
        ? deepmerge(styles, styles.darkMode)
        : styles
    let _imageStyles
    if (item.imageStyles) {
      _imageStyles =
        darkMode && !showScrolledNav
          ? deepmerge(item.imageStyles, item.imageStyles.darkMode)
          : item.imageStyles
    }
        ...
        navItem = (
          <NavItem
            ...
            <NavLink
              ...
              style={{
                ..._styles.navLink,
                color: showScrolledNav
                  ? _styles.navLink.color
                  : activeNavIndex === index
                    ? _styles.navLink.hover.color
                    : _styles.navLink.color,
                borderBottomColor:
                  activeNavIndex === index && !item.image
                    ? _styles.navLink.hover.borderBottomColor
                    : _styles.navLink.borderBottomColor,
              }}>
              {item.image ? (
                <Media
                  object
                  ...
                  style={
                    activeNavIndex === index
                      ? deepmerge(_imageStyles, _imageStyles.hover)
                      : _imageStyles
                  }
                />
              ) : (
            ...
            <DropdownToggle
              ...
              <div
                style={{
                  ...
                  color: showScrolledNav
                    ? _styles.dropdownItem.color
                    : activeNavIndex === index
                      ? _styles.dropdownItem.hover.color
                      : _styles.dropdownItem.color,
                  borderBottomColor:
                    activeNavIndex === index && !item.image
                      ? _styles.dropdownItem.hover.borderBottomColor
                      : _styles.dropdownItem.borderBottomColor,
                }}>
                {item.image ? (
                  <>
                    <Media
                      onClick={() => hoverNavItem(item, index)}
                      object
                      src={item.image}
                      style={
                        activeNavIndex === index
              ...
              {!item.image && (
                <Media
                  ...
                  style={
                    activeNavIndex === index
                      ? deepmerge(
                      _styles.dropdownIcon,
                      _styles.dropdownIcon.hover,
                      )
                      : _styles.dropdownIcon
                  }
  ...
  render() {
    ...
    const navImage =
      darkMode && !showScrolledNav && darkModeImg ? darkModeImg : activeNavImage
    ...
    return (
      <div
        ...
        <div
          ...
          style={{
            ...styles.gradient,
            opacity: showScrolledNav ? 0 : styles.gradient.opacity,
          }}></div>
        <Navbar
          id="navbar"
          expand="md"
          ...
          <Collapse id="collapse" navbar isOpen={open} style={_styles.collapse}>
            <Nav id="nav" navbar style={_styles.nav}>
              {items.map((item, index) =>
                renderNavigationItems(item, index, false),
              )}
            </Nav>
            <div
              id="imageItems"
              style={
                showScrolledNav
                  ? deepmerge(_styles.imageItems, _styles.imageItems.scrolled)
                  : _styles.imageItems
              }>
              {items.map((item, index) =>
                renderNavigationItems(item, index, true),
              )}
            </div>
          </Collapse>
        </Navbar>
